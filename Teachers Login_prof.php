...
if (isset($_POST['username'])){
	...
	//verificam daca user-ul exista sau nu in baza de date
     $query = "SELECT * FROM `profesori` WHERE nume_profesor='$username' and parola='$password'";
	$result = mysqli_query($con, $query) or die(mysql_error());
	$rows = mysqli_num_rows($result);

     if($rows == 1 && $rows3['aplicat'] == 1){
          header("Location: repartitie_finala.php");
     } elseif($rows==1 && $rows3['aplicat'] == 0){
	    $_SESSION['username'] = $username;
            // Redirect user to index.php
	    header("Location: manager_proiecte_prof.php");
         }else{
	echo "<div class='form'>
<h3>Username/password is incorrect.</h3>
<br/>Click here to <a href='Teachers Login_prof.php'>Login</a></div>";
	}
    }else{
?>
<div class="form">
...
<p>Are you a student? <a href='index.php'>Login Here</a></p>
</div>
<?php } ?>
</body>
</html>